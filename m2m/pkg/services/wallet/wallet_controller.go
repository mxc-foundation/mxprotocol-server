...
func (s *WalletServerAPI) GetWalletBalance(ctx context.Context, req *m2m.GetWalletBalanceRequest) (*m2m.GetWalletBalanceResponse, error) {
	...
	case auth.OrganizationIdRearranged:
		return &m2m.GetWalletBalanceResponse{UserProfile: &userProfile},
	...
	case auth.OK:
		...
		if err != nil {
			...
			return &m2m.GetWalletBalanceResponse{UserProfile: &userProfile}, nil
		}

		return &m2m.GetWalletBalanceResponse{Balance: balance, UserProfile: &userProfile}, nil
...
func (s *WalletServerAPI) GetVmxcTxHistory(ctx context.Context, req *m2m.GetVmxcTxHistoryRequest) (*m2m.GetVmxcTxHistoryResponse, error) {
	userProfile, res := auth.VerifyRequestViaAuthServer(ctx, s.serviceName, req.OrgId)
	...
	case auth.OrganizationIdRearranged:
		return &m2m.GetVmxcTxHistoryResponse{UserProfile: &userProfile},
			status.Errorf(codes.NotFound, "This organization has been deleted from this user's profile.")
	...
	case auth.OK:
		...
		return &m2m.GetVmxcTxHistoryResponse{UserProfile: &userProfile}, nil
	}

	return nil, status.Errorf(codes.Unknown, "")
}

func (s *WalletServerAPI) GetWalletUsageHist(ctx context.Context, req *m2m.GetWalletUsageHistRequest) (*m2m.GetWalletUsageHistResponse, error) {
	userProfile, res := auth.VerifyRequestViaAuthServer(ctx, s.serviceName, req.OrgId)
	...
	case auth.OrganizationIdRearranged:
		return &m2m.GetWalletUsageHistResponse{UserProfile: &userProfile},
			status.Errorf(codes.NotFound, "This organization has been deleted from this user's profile.")

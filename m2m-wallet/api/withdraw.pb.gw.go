...
func request_WithdrawService_GetWithdrawFee_0(ctx context.Context, marshaler runtime.Marshaler, client WithdrawServiceClient, req *http.Request, pathParams map[string]string) (proto.Message, runtime.ServerMetadata, error) {
	...
	val, ok = pathParams["cryptoCurrency"]
	if !ok {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "missing parameter %s", "cryptoCurrency")
	}

	e, err = runtime.Enum(val, CryptoCurrencies_value)

	if err != nil {
		return nil, metadata, status.Errorf(codes.InvalidArgument, "type mismatch, parameter: %s, error: %v", "cryptoCurrency", err)
	}

	protoReq.CryptoCurrency = CryptoCurrencies(e)
...
var (
	pattern_WithdrawService_GetWithdrawFee_0 = runtime.MustPattern(runtime.NewPattern(1, []int{2, 0, 2, 1, 1, 0, 4, 1, 5, 2}, []string{"api", "getWithdrawFee", "cryptoCurrency"}, "", runtime.AssumeColonVerbOpt(true)))
)
